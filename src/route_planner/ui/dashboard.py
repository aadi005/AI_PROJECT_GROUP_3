...
class VisualizerDashboard:
    def __init__(self, graph_path, scenarios_dir):
        ...
        self.scenarios_by_difficulty: dict[str, list[str]] = {"easy": [], "medium": [], "hard": []}
    ...
    def _build_layout(self):
        ...
        ttk.Label(
            outer,
            text="Choose difficulty/case, then run selected algorithm or benchmark all cases.",
            style="Hint.TLabel",
        ...
        self.scenario_combo = self._labeled_combo(controls, 0, 0, "Case", self.scenario_var, [])
        difficulty_combo = self._labeled_combo(
            controls,
            0,
            1,
            "Difficulty",
        ...
        difficulty_combo.bind("<<ComboboxSelected>>", self._on_difficulty_changed)
        self._labeled_combo(controls, 0, 2, "Algorithm", self.algorithm_var, ["bfs", "ucs", "greedy", "astar"])
        self._labeled_combo(controls, 0, 3, "Heuristic", self.heuristic_var, ["nearest", "mst"])
        ...
        ttk.Label(controls, text="Animation delay (sec)", style="Card.TLabel").grid(
            row=1, column=2, sticky=tk.W, padx=8, pady=(10, 0)
        )
        ttk.Scale(controls, from_=0.0, to=0.15, variable=self.delay_var, orient=tk.HORIZONTAL, length=220).grid(
            row=1, column=3, sticky=tk.W, padx=8, pady=(10, 0)
        )

        btns = ttk.Frame(outer, style="App.TFrame")
        btns.pack(fill=tk.X, pady=(12, 10))
        ttk.Button(btns, text="Run Selected", style="Action.TButton", command=self._run_selected).pack(
            side=tk.LEFT, padx=(0, 8)
        )
        ttk.Button(btns, text="Compare All", style="Action.TButton", command=self._run_compare).pack(
            side=tk.LEFT, padx=(0, 8)
        )
        ttk.Button(btns, text="Run All Cases", style="Action.TButton", command=self._run_all_cases).pack(
            side=tk.LEFT, padx=(0, 8)
        )
        ttk.Button(btns, text="Show Graph", style="Action.TButton", command=self._show_graph).pack(
            side=tk.LEFT, padx=(0, 8)
        )
    ...
    def _load_scenarios(self):
        self.scenarios_by_difficulty = self._collect_scenarios_by_difficulty()
        total_cases = sum(len(v) for v in self.scenarios_by_difficulty.values())
        if total_cases == 0:
    ...
    def _collect_scenarios_by_difficulty(self) -> dict[str, list[str]]:
        grouped = {"easy": [], "medium": [], "hard": []}
        for path in sorted(self.scenarios_dir.glob("*.json")):
            name = path.name
            if name.startswith("easy_"):
                grouped["easy"].append(name)
            elif name.startswith("medium_"):
                grouped["medium"].append(name)
            elif name.startswith("hard_"):
                grouped["hard"].append(name)
        return grouped

    def _refresh_case_dropdown(self) -> None:
        difficulty = self.difficulty_var.get().strip().lower()
        if difficulty == "all":
            options = sorted(
                [
                    *self.scenarios_by_difficulty.get("easy", []),
                    *self.scenarios_by_difficulty.get("medium", []),
                    *self.scenarios_by_difficulty.get("hard", []),
                ]
            )
        else:
            options = self.scenarios_by_difficulty.get(difficulty, [])
        self.scenario_combo.configure(values=options)
        self.scenario_var.set(options[0] if options else "")
    ...
    def _current_inputs(self):
        ...
        return graph, scenario, scenario_file

    def _run_selected(self):
        try:
            graph, scenario, _scenario_file = self._current_inputs()
            ...
            self._append(
                ...
                f"unique graph nodes touched: {len(set(result.explored_nodes))}\n"
    ...
    def _run_compare(self):
        try:
            ...
            if solved:
                ...
                for r in solved:
                    current_cost = r.metrics.weighted_path_cost
                    # Note: double check if your time variable is called 'runtime' or 'execution_time'
                    current_time = r.metrics.runtime_ms 
                    
                    if current_cost < lowest_cost:
                        lowest_cost = current_cost
                        lowest_time = current_time
                        best = r
                    elif current_cost == lowest_cost:
                        if current_time < lowest_time:
                            lowest_time = current_time
                            best = r
                
                # print(f"Debug: picked {best.algorithm} as best - cost: {lowest_cost}, time: {lowest_time}")
    ...
    def _run_all_cases(self):
        try:
            ...
            if difficulty == "all":
                ...
                scenario_files = list(self.scenarios_by_difficulty.get(difficulty, []))
            ...
            for scenario_file in scenario_files:
                ...
                errors = validate_scenario_against_graph(scenario, graph)
                if errors:
                    raise ValueError(f"Scenario validation failed for {scenario_file}:\n- " + "\n- ".join(errors))
                results = self.app.run_comparison(
                    graph=graph,
                    scenario=scenario,
                    heuristic_name=self.heuristic_var.get(),
                ...
                for result in results:
                    aggregate[result.algorithm].append(result)

            self._append(self._format_aggregate_summary(aggregate, difficulty, len(scenario_files)))
        except Exception as e:
            messagebox.showerror("Error", str(e))

    @staticmethod
    def _format_aggregate_summary(aggregate: dict[str, list], difficulty: str, case_count: int) -> str:
        headers = [
            "Algorithm",
            "Solved",
            "AvgCost",
        ...
        rows = []
        for algorithm in ("BFS", "UCS", "Greedy", "A*"):
            items = aggregate.get(algorithm, [])
            if not items:
                continue
            solved = [r for r in items if r.solved]
            costs = [r.metrics.weighted_path_cost for r in solved] or [0.0]
            expanded = [float(r.metrics.nodes_expanded) for r in items]
            runtime = [r.metrics.runtime_ms for r in items]
            rows.append(
                [
                    algorithm,
                    f"{len(solved)}/{len(items)}",
                    f"{mean(costs):.1f}",
                    f"{min(costs):.1f}",
                    f"{max(costs):.1f}",
                    f"{mean(expanded):.1f}",
        ...
        out = [f"Run All Cases ({difficulty}) · {case_count} scenarios", line(headers), divider]
    ...
    def _show_graph(self):
        try:
            graph = load_graph(self.graph_path)
            self._draw_map(graph, scenario=None, explored=None, path=None, title="Campus Graph", show_labels=True)
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def _draw_map(self, graph, scenario, explored, path, title, show_labels=False):
        ...
        if explored:
            counts = Counter(node for node in explored if node in graph.node_id_by_key)
            if counts:
                keys = list(counts.keys())
                eids = [graph.node_id(k) for k in keys]
                ex = [graph.nodes_by_id[i].x for i in eids]
                ey = [graph.nodes_by_id[i].y for i in eids]
                sizes = [22 + min(6, counts[k]) * 6 for k in keys]
                self.ax.scatter(
                    ex,
                    ey,
                    c="#f59e0b",
                    ...
                    alpha=0.8,
                    zorder=4,
                    label=f"Explored ({sum(counts.values())} states)",
                )
